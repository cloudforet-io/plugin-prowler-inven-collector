...
class AzureProwlerManager(CollectorManager):
    provider = "azure"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.cloud_service_type = "Azure-Standard"
    ...
    def collect(
        ...
        try:
            check_results = self.azure_prowler_connector.check(
    ...
    def make_compliance_results(self, check_results: List[dict]) -> List[dict]:
        ...
        for check_result in check_results:
            subscription_id = self.get_subscription_id_from_resource_id(
                check_result["ResourceId"]
            )
            compliance = check_result.get("Compliance", {})

            if len(compliance) == 0:
                check_id = check_result["CheckID"]
                azure_standard_id = f"prowler:azure:{subscription_id}:{self.cloud_service_type}:{check_id}".lower()
                status = check_result["Status"]
                severity = _SEVERITY_MAP.get(check_result["Severity"], "UNKNOWN")
                score = _SEVERITY_SCORE_MAP[severity]

                if azure_standard_id not in compliance_results:
                    compliance_results[
                        azure_standard_id
                    ] = self._make_azure_standard_compliance_result(
                        azure_standard_id,
                        severity,
                        check_result,
                        subscription_id,
                    ...
                    compliance_results[azure_standard_id]["data"]["checks"][
                        check_id
                    ] = self._make_check(check_result)

                # update severity
                compliance_results[azure_standard_id]["data"][
                    "severity"
                ] = self._update_severity(
                    compliance_results[azure_standard_id]["data"]["severity"], severity
                )

                compliance_results[azure_standard_id][
                    "data"
                ] = self._update_compliance_status_and_stats(
                    compliance_results[azure_standard_id]["data"], status, score
                )

                compliance_results[azure_standard_id]["data"]["checks"][
                    check_id
                ] = self._update_check_status_and_stats(
                    compliance_results[azure_standard_id]["data"]["checks"][check_id],
                    status,
                    score,
                )

                compliance_results[azure_standard_id]["data"]["findings"].append(
                ...
                self._make_compliance_result(
                    check_result, requirements, subscription_id, compliance_results
                )

        return self._convert_results(compliance_results)

    def _make_azure_standard_compliance_result(
        self,
        compliance_id: str,
        severity: str,
        check_result: dict,
        ...
        compliance_result = {
            "name": check_result["CheckTitle"],
            "reference": {
                "resource_id": compliance_id,
                "external_link": f"https://portal.azure.com/#@.onmicrosoft.com/resource{check_result.get('ResourceId')}/overview",
            },
            "data": {
                ...
                "resource_name": check_result.get("ResourceName"),
                "tenant": check_result.get("Tenant_Domain"),
                "subscription_name": check_result.get("Subscription"),
            },
            "metadata": {
                "view": {
                    "sub_data": {
    ...
    def _make_compliance_result(
        ...
        for requirement_id in requirements:
            compliance_id = f"prowler:azure:{subscription_id}:{self.cloud_service_type}:{requirement_id}".lower()
            check_id = check_result["CheckID"]
            status = check_result["Status"]
            region_code = check_result.get("Region")
            severity = _SEVERITY_MAP.get(check_result["Severity"], "UNKNOWN")
            ...
            if compliance_id not in compliance_results:
                compliance_results[
                    compliance_id
                ] = self._make_base_compliance_result_with_requirement_id(
    ...
    def _update_severity(old_severity: str, new_severity: str) -> str:
        if (
            _SEVERITY_SCORE_MAP[old_severity.upper()]
            < _SEVERITY_SCORE_MAP[new_severity.upper()]
        ):
            return new_severity
        return old_severity
    ...
    def _make_finding(check_result: dict) -> dict:
        return {
            ...
            "resource": check_result["ResourceId"],
    ...
    def _make_base_compliance_result_with_requirement_id(
    ...
    def _check_compliance_framework(self):
        all_compliance_frameworks = list(COMPLIANCE_FRAMEWORKS["azure"].keys())
    ...
    def _load_compliance_framework_info(self):
        compliance_framework = COMPLIANCE_FRAMEWORKS["azure"][self.cloud_service_type]
        compliance_frameworks = bulk_load_compliance_frameworks(self.provider)

        # Azure compliance framework not yet supported
        if compliance_framework not in compliance_frameworks:
            return
