...
from cloudforet.plugin.connector.google_prowler_connector import GoogleProwlerConnector
...
class GoogleProwlerManager(CollectorManager):
    provider = "google_cloud"

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.google_prowler_connector: GoogleProwlerConnector = (
    ...
    def collect(
        ...
        try:
            check_results = self.google_prowler_connector.check(
            ...
            cloud_service_type.metadata["query_sets"][0][
                "name"
            ] = f"Google Cloud {self.cloud_service_type}"
    ...
    def make_compliance_results(self, check_results: List[dict]) -> List[dict]:
        ...
        for check_result in check_results:
            account = check_result["ProjectId"]
            requirements = check_result.get("Compliance", {}).get(
                self.cloud_service_type, []
            )
            for requirement_id in requirements:
                compliance_id = f"prowler:google_cloud:{account}:{self.cloud_service_type}:{requirement_id}".lower()
                check_id = check_result["CheckID"]
                status = check_result["Status"]
                region_code = check_result["Location"]
    ...
    def _make_finding(check_result: dict) -> dict:
        return {
            ...
            "resource": check_result["ResourceId"] or check_result["ResourceName"],
    ...
    def _check_compliance_framework(self):
        all_compliance_frameworks = list(COMPLIANCE_FRAMEWORKS["google_cloud"].keys())
    ...
    def _load_compliance_framework_info(self):
        compliance_framework = COMPLIANCE_FRAMEWORKS["google_cloud"][
            self.cloud_service_type
        ]
        compliance_frameworks = bulk_load_compliance_frameworks("gcp")
